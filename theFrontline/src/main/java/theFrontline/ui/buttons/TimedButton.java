...
public class TimedButton extends ClickableUIElement {
    ...
    public TimedButton(float x, float y, float timer, Consumer<TimedButton> clickEffect, boolean ordered) {
        this(TextureLoader.getTexture(TheFrontline.makeUIPath("NormalButton.png")), x, y, timer, clickEffect, ordered);
    }
    ...
    public void setTotalOrder(int totalOrder) {
        if (this.totalOrder > -1 && totalOrder != this.totalOrder) {
            TheFrontline.logger.info("Tried to assign new total order to button. From: " + this.totalOrder + " to " + totalOrder);
            return;
        }
